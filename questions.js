...
function loadQuestion(index) {
  ...
  var currentTopic = "Story Reading and Analysis"; // Replace with dynamic topic fetching logic if needed
...
function displayResults() {
  ...
  for (var j = 0; j < questions.length; j++) {
    ...
    if (questions[j].options) {
      ...
      var answerColor = isCorrect ? "black" : "red"; // Set text color based on correctness
      givenAnswer = `<span style="color:${answerColor}">${givenAnswer}</span>`;
      var passage = questions[j].passage ? "<br><b>Passage:</b> " + questions[j].passage + "<br>" : "";
